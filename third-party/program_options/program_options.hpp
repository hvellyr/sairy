...
public:
    explicit invalid_bool_value(const std::string& value)
        : option_parse_error("", value, "invalid bool value '" + value + "', accepts: (yes|no)")
    { }
};
...
class option_adder {
public:
    option_adder(options_description* owner)
        : owner_(owner)
    { }
...
private:
    ...
    std::string format_desc(const std::shared_ptr<const option_description>& o, size_t indent, size_t width) const;
...
inline std::string options_description::format_desc(const std::shared_ptr<const option_description>& o, size_t indent, size_t width) const
{
                   ...
                   = positional_options_description())
{
    ...
    auto get_next_arg = [&argc, &argv, &is_option](const std::string& opt,
         const std::shared_ptr<const value_semantic>& semantic, int& idx) -> const char*
    {
...
inline void store(parsed_options&& options, variables_map& vm)
{
