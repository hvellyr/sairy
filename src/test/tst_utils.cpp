...
namespace utils {
  ...
  TEST_CASE("Make relative paths", "[utils]") {
    REQUIRE(fs::path("assets/pic.png") ==
            make_relative(fs::path("data/"), fs::path("data/assets/pic.png")));
  }


  TEST_CASE("Relative paths with volumes", "[utils]") {
    REQUIRE(fs::path("assets/pic.png") ==
            make_relative(fs::path("c:/data/"), fs::path("c:/data/assets/pic.png")));
  }
  ...
  TEST_CASE("trim_right", "[string][utils]") {
    ...
    REQUIRE(string("   hello world") == trim_right_copy("   hello world   "));
    REQUIRE(string("hello world") == trim_right_copy("hello world   "));
    REQUIRE(string("\0x09hello   world") ==
            trim_right_copy("\0x09hello   world\0x09   "));
    REQUIRE(string("\n\rhello   world") ==
            trim_right_copy("\n\rhello   world\0x09   \n"));
  }
  ...
  TEST_CASE("split", "[string][utils]") {
    ...
    REQUIRE((vector<string>{"abc", "def", "x"}) ==
            split("  abc ;  def ;      x  ", ";", true));
    REQUIRE((vector<string>{"abc", "def", "", "x"}) ==
            split("  abc ;  def ; ;   x  ", ";", true));
  }
