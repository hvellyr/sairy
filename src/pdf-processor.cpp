...
namespace {
  ...
  void escape_str_to_stream(std::ostream& os, const std::string& str,
                            pdf_detail::PdfStyleContext& ctx)
  {
    ...
    for (const auto c : str16) {
      ...
      case '\n':
        ...
        case pdf_detail::k_asis_wrap:
          os << "\\\\\\relax" << std::endl;
          break;
        case pdf_detail::k_normal_wrap:
          os << " ";
          break;
        case pdf_detail::k_no_wrap:
          os << " ";
          break;
        }
        break;
      ...
      case '\t':
        ...
        case pdf_detail::k_preserve_ws:
          os << "\\ws{}";
          break;
        case pdf_detail::k_collapse_ws:
          os << ' ';
          break;
        case pdf_detail::k_ignore_ws:
          break;
        }
        break;
  ...
  void vspace(PdfProcessor* po, const IFormattingObject* fo,
  ...
  void enc_fontsize(PdfProcessor* po, const IFormattingObject* fo)
  {
    ...
    if (fontcaps == "normal")
      po->style_ctx()._capsstyle = pdf_detail::k_normal_caps;
    else if (fontcaps == "lower")
      po->style_ctx()._capsstyle = pdf_detail::k_lower_caps;
    else if (fontcaps == "caps")
      po->style_ctx()._capsstyle = pdf_detail::k_upper_caps;
    else if (fontcaps == "small-caps")
      po->style_ctx()._capsstyle = pdf_detail::k_small_caps;
  ...
  void enc_paraprops(PdfProcessor* po, const IFormattingObject* fo)
  {
    double max_inf = std::numeric_limits<double>::infinity();
  ...
  void enc_color(PdfProcessor* po, const fo::Color co)
  {
    switch (co._space) {
    case fo::kRGB:
      po->stream() << "\\color[rgb]{"
  ...
  class PdfLiteralFoProcessor : public IFoProcessor<PdfProcessor> {
  public:
    void render(PdfProcessor* po, const IFormattingObject* fo) const override
    {
      po->finalize_breaks();
      auto str = static_cast<const fo::Literal*>(fo)->text();

      switch (po->style_ctx()._capsstyle) {
      case pdf_detail::k_normal_caps:
        escape_str_to_stream(po->stream(), str, po->style_ctx());
        break;
      case pdf_detail::k_lower_caps:
        escape_str_to_stream(po->stream(), boost::to_lower_copy(str),
                             po->style_ctx());
        break;
      case pdf_detail::k_upper_caps:
        escape_str_to_stream(po->stream(), boost::to_upper_copy(str),
                             po->style_ctx());
        break;
      case pdf_detail::k_small_caps:
        po->stream() << "{\\sc ";
        escape_str_to_stream(po->stream(), str, po->style_ctx());
        po->stream() << "}";
        break;
  ...
  class PdfParagraphFoProcessor : public IFoProcessor<PdfProcessor> {
  public:
    void render(PdfProcessor* po, const IFormattingObject* fo) const override
    {
      po->finalize_breaks();
      ...
      if (linesprops == "asis")
        po->style_ctx()._wrapstyle = pdf_detail::k_asis_wrap;

      auto wstreatment = po->property(fo, "whitespace-treatment",
                                      std::string("collapse"));
      if (wstreatment == "preserve")
        po->style_ctx()._wstreatment = pdf_detail::k_preserve_ws;
      else if (wstreatment == "collapse")
        po->style_ctx()._wstreatment = pdf_detail::k_collapse_ws;
      else if (wstreatment == "ignore")
        po->style_ctx()._wstreatment = pdf_detail::k_ignore_ws;
  ...
  class PdfParagraphBreakFoProcessor : public IFoProcessor<PdfProcessor> {
  public:
    void render(PdfProcessor* po, const IFormattingObject* fo) const override
    {
      po->finalize_breaks();
  ...
  class PdfDisplayGroupFoProcessor : public IFoProcessor<PdfProcessor> {
  public:
    void render(PdfProcessor* po, const IFormattingObject* fo) const override
    {
      if (auto break_before = po->property_or_none<std::string>(fo, "break-before?")) {
        if (*break_before == "page")
          po->request_page_break(pdf_detail::kBreakPageBefore);
      }
      ...
      if (auto break_after = po->property_or_none<std::string>(fo, "break-after?")) {
        if (*break_after == "page")
          po->request_page_break(pdf_detail::kBreakPageAfter);
      }
    }
  };


  class PdfSimplePageSequenceFoProcessor : public IFoProcessor<PdfProcessor> {
  public:
    void render(PdfProcessor* po, const IFormattingObject* fo) const override
    {
      if (!po->_first_page) {
      ...
      po->request_page_break(pdf_detail::kNoBreak);
      ...
      po->request_page_break(pdf_detail::kSurpressNextPageBreak);
      po->render_sosofo(&fo->port("text"));
    }
  };


  class PdfSequenceFoProcessor : public IFoProcessor<PdfProcessor> {
  public:
    void render(PdfProcessor* po, const IFormattingObject* fo) const override
    {
      po->finalize_breaks();
  ...
  class PdfLineFieldFoProcessor : public IFoProcessor<PdfProcessor> {
  public:
    void render(PdfProcessor* po, const IFormattingObject* fo) const override
    {
      po->finalize_breaks();
  ...
  class PdfPageNumberFoProcessor : public IFoProcessor<PdfProcessor> {
  public:
    void render(PdfProcessor* po, const IFormattingObject* fo) const override
    {
      po->finalize_breaks();
  ...
  class PdfAnchorFoProcessor : public IFoProcessor<PdfProcessor> {
  public:
    void render(PdfProcessor* po, const IFormattingObject* fo) const override
    {
      po->finalize_breaks();
  ...
  class PdfSimpleColumnSetSequenceProcessor : public IFoProcessor<PdfProcessor> {
  public:
    void render(PdfProcessor* po, const IFormattingObject* fo) const override
    {
      po->finalize_breaks();
...
PdfProcessor::PdfProcessor()
  : _verbose(false),
    _style_ctx{pdf_detail::k_normal_caps, pdf_detail::k_normal_wrap,
               pdf_detail::k_collapse_ws},
    _cropmarks(pdf_detail::kOff),
...
PdfProcessor::PdfProcessor(const po::variables_map& args) : PdfProcessor()
{
  if (!args.empty()) {
    ...
    if (args.count("cropmarks")) {
      ...
      if (cropmarks == "cam")
        _cropmarks = pdf_detail::kCamera;
      else if (cropmarks == "frame")
        _cropmarks = pdf_detail::kFrame;
      else
        _cropmarks = pdf_detail::kOff;
    }
...
std::string PdfProcessor::proc_id() const
{
  return "pdf";
}


std::string PdfProcessor::default_output_extension() const
{
  return ".pdf";
}

po::options_description PdfProcessor::program_options() const
{
  std::string opts_title =
    std::string("Pdf renderer [selector: '") + proc_id() + "']";
...
PdfProcessor::lookup_fo_processor(const std::string& fo_classname) const
{
  static auto procs =
    std::map<std::string, std::shared_ptr<IFoProcessor<PdfProcessor>>>{
      {"#literal", std::make_shared<PdfLiteralFoProcessor>()},
      {"#paragraph", std::make_shared<PdfParagraphFoProcessor>()},
      {"#paragraph-break", std::make_shared<PdfParagraphBreakFoProcessor>()},
      {"#display-group", std::make_shared<PdfDisplayGroupFoProcessor>()},
      {"#simple-page-sequence",
       std::make_shared<PdfSimplePageSequenceFoProcessor>()},
      {"#sequence", std::make_shared<PdfSequenceFoProcessor>()},
      {"#line-field", std::make_shared<PdfLineFieldFoProcessor>()},
      {"#page-number", std::make_shared<PdfPageNumberFoProcessor>()},
      {"#anchor", std::make_shared<PdfAnchorFoProcessor>()},
      {"#simple-column-set-sequence",
       std::make_shared<PdfSimpleColumnSetSequenceProcessor>()}
    };
...
void PdfProcessor::before_rendering()
{
...
void PdfProcessor::after_rendering()
{
  _stream << "\\end{document}" << std::endl;
}


boost::filesystem::ofstream& PdfProcessor::stream()
{
  return _stream;
}

pdf_detail::PdfStyleContext& PdfProcessor::style_ctx()
{
  return _style_ctx;
}


void PdfProcessor::finalize_breaks()
{
  switch (_break_pending) {
  case pdf_detail::kSurpressNextPageBreak:
  case pdf_detail::kNoBreak:
    _break_pending = pdf_detail::kNoBreak;
    break;
  case pdf_detail::kBreakPageBefore:
  case pdf_detail::kBreakPageAfter:
    stream() << std::endl
             << "\\newpage{}" << std::endl
...
void PdfProcessor::request_page_break(pdf_detail::BreakKind breakKind)
{
  if (breakKind != _break_pending &&
      _break_pending != pdf_detail::kSurpressNextPageBreak) {
    _break_pending = breakKind;
  }
}


fo::LengthSpec PdfProcessor::paper_width() const
{
  return std::get<0>(_paper_dimen);
}


fo::LengthSpec PdfProcessor::paper_height() const
{
