...
namespace textbook {
  ...
  public:
    TagSpec()
      : _is_env(false), _is_mixed(false), _starts_p(false), _is_block(false)
    {
    }

    TagSpec(const std::string& tagnm, const std::vector<AttrSpec>& attrspecs,
            std::tuple<size_t, size_t> attrc, bool is_env = false,
            bool is_mixed = false, bool starts_p = false, bool is_block = false)
      : _tagnm(tagnm), _attrspecs(attrspecs), _attrc(attrc), _is_env(is_env),
        _is_mixed(is_mixed), _starts_p(starts_p), _is_block(is_block)
    {
    }

    bool is_env() const { return _is_env; }
    bool is_mixed() const { return _is_mixed; }
    bool starts_p() const { return _starts_p; }
    bool is_block() const { return _is_block; }

    std::string tag_name() const { return _tagnm; }

    size_t min_args() const { return std::get<0>(_attrc); }
    size_t max_args() const { return std::get<1>(_attrc); }

    std::vector<AttrSpec> attrspecs() const { return _attrspecs; }
  };
